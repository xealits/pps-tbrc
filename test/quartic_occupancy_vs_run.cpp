...
int main(int argc, char* argv[]) {
  ...
  int num_triggers = 0;
  for (int sp=1; sp<1000000; sp++) { // we loop over all spills
    ...
    try {
      ...
      while (true) {
        ...
        if (num_triggers>=trigger_start and e.GetType()==VME::TDCEvent::TDCMeasurement and !e.IsTrailing()) occ->FillChannel(e.GetChannelId(), 1);
      }
